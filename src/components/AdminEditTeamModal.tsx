...
import { User, Users, Layers, AlertCircle, Save, ShieldCheck } from 'lucide-react';
...
export function AdminEditTeamModal({
  team,
  isOpen,
  onClose,
  onSuccess,
  ...
  // Extract non-lead members (5 members)
  ...
  const handleReviewEdits = () => {
    ...
    if (!trimmedTeamName || !trimmedTeamName.endsWith('_RGUKTN')) {
      setErrorMsg('Team Name is mandatory and must end exactly with "_RGUKTN".');
      return;
    }
    ...
    if (!leadEmail.trim().toLowerCase().endsWith('@rguktn.ac.in')) {
      setErrorMsg('Team Leader email must end with @rguktn.ac.in.');
      return;
    }
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      if (!m.name?.trim() || !m.id_number?.trim() || !m.email?.trim() || !m.phone?.trim()) {
        setErrorMsg(`All details are required for Team Member #${num}.`);
        return;
      }
      if (!m.email.trim().toLowerCase().endsWith('@rguktn.ac.in')) {
        setErrorMsg(`Team Member #${num} email must end with @rguktn.ac.in.`);
        return;
      }
    }

    const allMembersList = [{ gender: leadGender }, ...members];
    ...
    if (diffs.length === 0) {
      setErrorMsg('No changes detected. Please modify at least one field before saving.');
      return;
    }
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Admin error updating team registration.');
      }
      ...
      onClose();
      alert('Team details updated successfully. Only the changed information was modified.');
    } catch (err: any) {
      console.error('Admin edit submission error:', err);
      setErrorMsg(err.message || 'Failed to update team details.');
      setIsDiffModalOpen(false);
    } finally {
  ...
  return (
    <>
      <Modal
        isOpen={isOpen}
        onClose={onClose}
        title={`Admin Team Editor — ${team.team_id}`}
        maxWidth="4xl"
      >
        <div className="space-y-6">
          
          <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between text-xs text-amber-900 font-semibold">
            <span className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-amber-700" /> Admin Override Mode: Surgical update enabled.
            </span>
            <span className="text-[10px] font-bold bg-amber-200 text-amber-900 px-2 py-0.5 rounded">ADMIN</span>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                  <p className="mt-1 text-[10px] text-amber-700 font-bold">Must end with "_RGUKTN".</p>
                </div>
          ...
          <div className="flex justify-between items-center pt-4 border-t border-slate-100">
            ...
            <button
              ...
              className="px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow transition-all flex items-center gap-2 cursor-pointer"
            >
              <Save className="w-4 h-4" /> Save Admin Edits
            </button>
          </div>
